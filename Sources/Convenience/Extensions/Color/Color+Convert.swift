...
//
    ...
    extension NSColor {
        ...
        /// Creates a new NSColor instance from hex representation.
        public convenience init?(hex: String) {
            guard Regex.color().match(hex).isSuccess else { return nil }
            var color = hex.replacingOccurrences(of: "#", with: "")
            if color.count == 6 { color += "FF" }
        ...
        public static func hex(_ value: String) -> NSColor? { NSColor(hex: value) }
        
        public static func hex(rgb value: Int) -> NSColor? { NSColor(rgb: value) }
        
        public static func hex(rgba value: UInt32) -> NSColor? { NSColor(rgba: value) }
